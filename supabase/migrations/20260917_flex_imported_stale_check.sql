-- ibkr_sync_stalled (2026-08-23) only proves the FETCH side is alive.
-- dcb5bdba fetched successfully every day for weeks while nothing was ever
-- imported into trades, because import only ever runs client-side
-- (_flexImportFromCache, triggered by the user opening the app) — ibkr-cron
-- only writes flex_statement_cache.xml/fetched_at and always sets
-- imported_at: null. A user who stops opening the app has a permanently
-- green "sync ok" status while their journal silently falls behind; nothing
-- existing caught this until a manual reconciliation did (see
-- 20260917_recover_dcb5bdba_ibkr_import.sql).
--
-- Uses stale_since/confirm_stale_since (20260917_flex_stale_since_tracking.sql),
-- not fetched_at/imported_at directly — a naive staleness test on those two
-- columns false-positives on every healthy account in the ordinary gap
-- between a cron fetch and the user's next visit, since ibkr-cron resets
-- imported_at to null on every fetch by design.
--
-- IMPORTANT: this function is rebuilt in full on every change (CREATE OR
-- REPLACE has no partial-alter form). Base this off the actual current
-- definition (pg_get_functiondef), not off an older migration file picked by
-- eye — an earlier attempt at this exact migration reconstructed the body
-- from the 20260823 file and silently reintroduced the pre-fix half_closed_row
-- definition (20260827) and dropped bybit_sync_stalled (20260826) and
-- opposite_direction_open_same_symbol (20260831). This version is the
-- 20260831 body verbatim plus one appended branch.
